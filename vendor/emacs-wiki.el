...
(defsubst emacs-wiki-published-name (name &optional current)
  ...
  (setq name (emacs-wiki-transform-name name))
  (when current
...
outsiders.
 ...
 verbatim
   Protects against highlighting and wiki interpretation, and escapes
   HTML content.
...
(defsubst emacs-wiki-link-properties (help-str &optional face)
  (append (if face
	      (list 'face face 'rear-nonsticky
                  '(emacs-wiki-keymap-property))
	    (list 'invisible t 'intangible t 'rear-nonsticky
                  '(emacs-wiki-keymap-property invisible intangible)))
